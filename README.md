...
**Chain and Baseline** create one persistent dimension set rather than a stack of unrelated objects. Pick the datum, then each subsequent point; every accepted member is its own undo step. Chain members join end to end on one shared line. Baseline members reuse the datum and add automatically spaced rows derived from label size. Select the set to change its shared offset or spacing, expand its manager/Selected Dimension member list, reattach one member endpoint, insert a point into a chain, or delete a member. Short chain labels alternate outside their segment when inline placement would collide. Set members keep independent Live, Fallback, or Needs Repair states, and valid sets generate through Grease Pencil, SVG, and PDF output.
...
**Renderable output:** open **Output** in the Dimensions sidebar, choose Selected or Visible annotations, then use the **Grease Pencil** section and choose Camera Relative or World Scale sizing. Camera Relative uses the active camera and resolves pixel sizes at each annotation's depth; World Scale uses explicit scene-unit sizes. Click **Generate Grease Pencil Output** to create renderable linear, angle, area, radial, diameter, and arc-length strokes in the scene-owned `Dimensions Output` collection. Generated objects use 3D Location depth ordering with Grease Pencil lighting disabled, and generation enables the active view layer's Depth and Grease Pencil data passes. Fallback and Needs Repair bindings are skipped until repaired. Labels and presentation offsets follow the live annotations. Live annotations remain the source of truth. Generated objects are disposable: regenerating the same annotation replaces its prior output and any hand edits to that generated object.
...
- Grease Pencil generation supports linear, angle, area, radial, diameter, and arc-length annotations in valid Live or Captured state. Measurements and construction guides remain viewport/construction data.
