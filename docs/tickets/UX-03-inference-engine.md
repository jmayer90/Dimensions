...
Candidate selection first ranks derived results by screen distance, inference type, and stable generation order. Native geometry owns the full configured snap aperture so an active-plane or line inference cannot shrink ordinary snapping; explicitly locking an inference reference is the deliberate override. Disabled Edge, Guide, or Face targets suppress references derived from that source before generation. Exact-topology acquisitions (angle/source faces and guided repair) deliberately stay geometry-only.
